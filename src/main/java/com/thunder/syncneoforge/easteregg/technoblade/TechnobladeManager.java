package com.thunder.syncneoforge.easteregg.technoblade;
    ...
    private static void transformEntityIntoTechnoblade(Entity entity, boolean shouldAnnounce) {
        if (((TechnobladeTransformable)entity).transformIntoTechnoblade()) {
            ...
            if (shouldAnnounce && Sync.getConfig().allowTechnobladeAnnouncements()) {
                MinecraftClient.getInstance().inGameHud.getChatHud().addMessage(Text.translatable("multiplayer.player.joined", ((TechnobladeTransformable)entity).asTechnoblade().getDisplayName()));
            }
        }
    }
