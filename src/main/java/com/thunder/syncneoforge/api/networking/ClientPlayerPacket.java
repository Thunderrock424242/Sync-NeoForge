package com.thunder.syncneoforge.api.networking;
...
public interface ClientPlayerPacket extends PlayerPacket {
    ...
    static <T extends ClientPlayerPacket> void register(Class<T> type) {
        ...
        boolean originalExecute = ClassUtil.getMethod(type, "execute", MinecraftClient.class, ClientPlayNetworkHandler.class, PacketSender.class).map(x -> "ClientPlayerPacket".equals(x.getDeclaringClass().getName())).orElse(false);
